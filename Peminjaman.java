...
public class Peminjaman extends BorderPane{
	...
	PasswordField pass;
	Button tombol = new Button("Pinjam");
	...
	public Peminjaman() {
		...
		tombol.setOnAction(e -> {
			if(idMember.getText().length() == 0 || idBuku.getText().length() == 0 || idStaff.getText().length() == 0 || namaBuku.getText().length() == 0 || pass.getText().length() == 0) {
				...
				alert.setHeaderText("Harap mengisi informasi peminjaman dengan lengkap!");
				alert.showAndWait();
			}
			else {
				Alert alert = new Alert(AlertType.INFORMATION);
				alert.setHeaderText("Informasi Penyimpanan berhasil disimpan");
				alert.showAndWait();
			}
		});
